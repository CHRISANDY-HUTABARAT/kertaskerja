...
@section('title', 'Admin - PSAK Government')
...
<div class="min-h-screen" style="background:#f1f5f9;">
    <div class="max-w-7xl mx-auto px-8 py-10">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <h1 class="text-2xl font-black tracking-tight text-slate-900 leading-none uppercase">
                            PSAK <span class="text-red-600">Government</span>
                        </h1>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
            ...
            <form action="{{ route('admin.psak.gov.store') }}" method="POST">
                @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-8 py-5 border-b border-slate-100">
                <div class="flex items-center space-x-3 mb-5">
                    <div class="w-1.5 h-8 bg-red-600 rounded-full"></div>
                    <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Riwayat Data PSAK Government</h2>
                </div>
                <form method="GET" action="{{ route('admin.psak.gov') }}" class="grid grid-cols-4 gap-3">
                    <div>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Comm (SSL)</th>
                            <th class="px-6 py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Real (SSL)</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($govs as $psak)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 text-sm font-bold text-slate-400">{{ $govs->firstItem() + $loop->index }}</td>
                            ...
                            <td class="px-6 py-4 text-sm font-bold text-slate-800">{{ $segmentNames[$psak->segment] ?? $psak->segment }} </td>
                        ...
                        <tr>
                            <td colspan="8" class="py-16 text-center">
                                ...
                                <p class="text-sm font-bold text-slate-400">Belum ada data PSAK Government</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="px-8 py-4 border-t border-slate-100 flex items-center justify-between">
                <p class="text-xs font-semibold text-slate-400">
                    Menampilkan {{ $govs->firstItem() }}–{{ $govs->lastItem() }} dari {{ $govs->total() }} data
                </p>
                <div class="flex items-center gap-1">
                    ...
                    @else
                        <a href="{{ $govs->previousPageUrl() }}" class="px-3 py-1.5 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">‹</a>
                    @endif
                    @foreach($govs->getUrlRange(1, $govs->lastPage()) as $page => $url)
                        @if($page == $govs->currentPage())
                        ...
                        @endif
                    @endforeach
                    @if($govs->hasMorePages())
                        <a href="{{ $govs->nextPageUrl() }}" class="px-3 py-1.5 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">›</a>
                    @else
                        <span class="px-3 py-1.5 text-xs font-bold text-slate-300 bg-slate-50 border border-slate-200 rounded-lg cursor-not-allowed">›</span>
                    @endif
                </div>
